...
def getter(widget):
    ...
    Notes:
        截屏(大小为鼠标选定区域大小长宽各拓宽200像素/若app窗口左上角超出屏幕范围,则从屏幕左上角开始截屏),
...
def onLeftButtonDown(event):
    ...
    Notes:
        由鼠标左键选择工具栏->文本触发,以鼠标坐标为左上角原点创建文本框等待用户输入内容,
...
def Open():
    ...
    Notes:
       导入指定本地图片,调整为指定大小(800,600),以指定坐标(400,300)为左上角原点插入
    """ 
...
def chooseBackColor():
    """ask to choose background color
    """    
    global backColor
    backColor = tkinter.colorchooser.askcolor()[1]

def chooseColor():
    """ask to choose paint brush color
    """    
...
def chooseThickness():
    """ask to choose the thickness of the paint brush
